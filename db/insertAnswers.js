...
let insertData = async (data, callback, callback2, timeStart) => {
  ...
  let currentQuery = `INSERT INTO answers (${keys}) VALUES ${valueList};`
...
let insertAnswers = async () => {
  for (let i = 1; i < 801; i++) {
    await getData(`./db/dataGeneration/data/answers/25KAnswer${i}.json`, (start) => {
      let end = Date.now()
      let elapsed = (end - start) / 1000
      console.log(`Inserted 25KAnswer${i}.json in ${elapsed} seconds`)
    })
  }
}
